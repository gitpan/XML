package XML2::DOM::Document;

use strict;
use warnings;

use XML2::Element::DocumentType;
use XML2::Attribute::Namespace;
use XML2;
use Carp;

=head2 createDocumentType

$doctype = XML2::Type->new( $qualifiedName, $publicId, $systemId );
...
sub createDocumentType
{
	...
	my $doctype = XML2::Element::DocumentType->new(
		name     => $qualifiedName,
		publicid => $publicId,
		systemid => $systemId,
		dtd      => $dtd,
...
$xml = XML2->new( $namespaceURI, $qualifiedName, $doctype );
...
sub createDocument
{
	...
	$doctype ||= XML2->createDocumentType;
...
sub documentElement
{
    ...
    if(not $self->{'element'}) {
		if($setObj) {
			...
			$self->{'element'} = $setObj;
			if(ref($setObj) eq 'XML2::Element::Document') {
				$self->{'fragment'} = 1;
			}
		} else {
...
sub getNamespace
{
	...
	if($uri eq 'xmlns' and not $self->{'xmlns'}->{'xmlns'}) {
		$self->{'xmlns'}->{'xmlns'} = XML2::Attribute::Namespace->new(
			owner => $self,
			name => 'xmlns',
			prefix => 'xmlns',
